...
export default function MediaLibraryModal({ 
  ...
  const getFileTypeLabel = (item: MediaItem) => {
    const extension = item.file_name.split('.').pop()?.toUpperCase();
    ...
    if (item.mime_type.includes('word') || item.mime_type.includes('document')) return 'DOC';
    if (item.mime_type.includes('csv') || item.mime_type.includes('spreadsheet')) return 'SHEET';
